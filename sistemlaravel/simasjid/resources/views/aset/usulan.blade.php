...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1><i class="fa fa-lightbulb"></i> Usulan Aset</h1>
        </div>
        <div class="row" style="padding: 10px;">
            <div class="col-12">
                <a href="#" class="btn btn-lg btn-info btn-primary" data-toggle="modal" data-target="#tambahModal"><i class="fas fas fa-plus"></i> Tambah Usulan Baru</a>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                    ...
                    <table id="table_id" class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th class="dt-center">Nama Barang Usulan</th>
                                ...
                                @if($inside_pengelola)
                                <th class="dt-center" style="width: 11em">Pelaksanaan</th>
                                @endif
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $usulan_dalam_list->nama }}</td>
                                <td>{{ $usulan_dalam_list->jenis_aset->nama }}</td>
                                <td>{{ $usulan_dalam_list->kategori->nama }}</td>
                                <td class="dt-center" style="width: 50px;padding-right: 5px;padding-left: 5px;height: 50px;">{{ $usulan_dalam_list->jumlah }}</td>
                                <!-- <td>{{ $usulan_dalam_list->harga }}</td>
                                <td>{{ $usulan_dalam_list->harga * $usulan_dalam_list->jumlah }}</td> -->
                                <td class="dt-center">{{ $usulan_dalam_list->created_at->format('d-M-Y H:i:s') }}</td>
                                <td class="dt-center">{{ $usulan_dalam_list->updated_at->format('d-M-Y H:i:s') }}</td>
                                <!-- <td>{{ $usulan_dalam_list->updated_at->diffForHumans() }}</td> -->
                                <td class="font-status">{!!$usulan_dalam_list->status_usulan!!}</td>
                                <td class="dt-center">
                                    <!-- <div class="btn-group mb-3" role="group" aria-label="Basic example" style="padding-left: 20px;"> -->
                                    <a href="#" class="open-detail btn btn-icon btn-sm btn-info" data-toggle="modal" data-id="{{ $usulan_dalam_list->id }}" data-target="#detailModal" style="width:100%"><i class="fas fa-glasses"></i> Detail</a>
                                    @if($inside_pengelola && ($usulan_dalam_list->status_usulan == "Menunggu Keputusan"))
                                    <a href="#" class="open-edit btn btn-icon btn-sm btn-warning" data-toggle="modal" data-id="{{ $usulan_dalam_list->id }}" data-target="#editModal" style="width:100%"><i class="fas fa-edit"></i></i> Edit</a>
                                    <a href="#" class="open-delete btn btn-icon btn-sm btn-danger" data-toggle="modal" data-id="{{ $usulan_dalam_list->id }}" data-target="#deleteModal" style="width:100%"><i class="fas fa-trash"></i> Hapus</a>
                                    @endif
                                    <!-- </div> -->
                                </td>
                                @if($inside_pengelola)
                                <td class="dt-center">
                                    @if($inside_pengelola && ($usulan_dalam_list->status_usulan == "Menunggu Keputusan"))
                                    <form class="form-inline" role="form" action="{{ route('usulanUpdate') }}" method="post">
                                        @csrf
                                        <div class="form-group">
                                            <input id="id" type="text" class="form-control" name="id" value="{{ $usulan_dalam_list->id }}" readonly hidden>
                                            <input id="id_pengelola" type="text" class="form-control" name="id_pengelola" value="{{$anggota->id}}" readonly hidden>
                                        </div>
                                        <div style="text-align: center; margin:auto;">Keputusan</div>
                                        <div class="form-group">
                                           
                                            <select class="form-control" id="keputusan" name="keputusan" style="width: 13em;" required>
                                                <option value="Diterima">Diterima</option>
                                                <option value="Ditolak">Ditolak</option>
                                            </select>
                                        </div>
                                        <button type="submit" class="btn btn-primary" style="margin: 5px; width: 100%;">OK</button>
                                    </form>
                                    @endif
                                    @if($inside_pengelola && ($usulan_dalam_list->status_usulan == "Diterima"))
                                    <form class="form-inline" role="form" action="{{ route('pembelianCreate') }}" method="post">
                                        @csrf
                                        <div class="form-group">
                                            <input id="id" type="text" class="form-control" name="id" value="{{ $usulan_dalam_list->id }}" readonly hidden>
                                            <input id="id_pengelola" type="text" class="form-control" name="id_pengelola" value="{{$anggota->id}}" readonly hidden>
                                        </div>
                                        <button type="submit" class="btn btn-primary" style="margin: 5px; width: 100%;"><i class="fas fa-shopping-bag"></i> Buat Pembelian</button>
                                    </form>
                                    @endif
                                </td>
                                @endif
        ...
        $('#table_id').DataTable({
            ...
            initComplete: function() {
                this.api().columns([1]).every(function() {
                    var column = this;
                    var input = $('<input class="form-control select" placeholder="Filter Nama Barang Usulan" style="margin-bottom:10px;"></input>')
