...
var mappingEditNotulensi = server+"/notulensi/detail/";
...
function updateKeterangan(idPekerjaan, jenis) {
    console.log("id keterangan : "+idPekerjaan+". jenis : "+jenis);
    if (jenis == 0){
        var contentProgress = document.getElementById("progres"+idPekerjaan).value;
        var sethtml = ''+contentProgress;

        $(".kontendariprogres"+idPekerjaan).empty();
        $(".kontendariprogres"+idPekerjaan).append(sethtml);
        $(".hasilkontendariprogres"+idPekerjaan).empty();
        $(".hasilkontendariprogres"+idPekerjaan).append(sethtml);
    } else if(jenis == 1){
        var contentProgress1 = document.getElementById("progres"+idPekerjaan).value;
        var contentProgress2 = document.getElementById("masukkan"+idPekerjaan).value;
        ...
        $(".kontendarimasukkan"+idPekerjaan).empty();
        $(".kontendarimasukkan"+idPekerjaan).append(sethtml);
        $(".hasilkontendariprogres"+idPekerjaan).empty();
        $(".hasilkontendariprogres"+idPekerjaan).append(sethtml);
    } else {
        var contentProgress = document.getElementById("keputusan"+idPekerjaan).value;
        var sethtml = contentProgress;
        $(".hasilkontendarimasukkan"+idPekerjaan).empty();
        $(".hasilkontendarimasukkan"+idPekerjaan).append(sethtml);
    }
}

function tambahKeterangan(nilai, jenis){
    console.log('id : '+ nilai + '. keputusan -> '+ jenis);
    if(jenis == 0) {
...
function simpanProgres() {
    ...
    var detailProgresId = document.getElementsByClassName("detailProgresId");
    var idPekerjaan = document.getElementsByClassName("idPekerjaan");
    var idNotulensi = document.getElementById("idNotulensi").value;

    var contentProgress = document.getElementsByClassName("progres");
    var contentMasukkan = document.getElementsByClassName("masukkan");
    var contentKeputusan = document.getElementsByClassName("keputusan");

    console.log("panjang id detail :  "+ detailProgresId.length);

    for (index = 0, len = detailProgresId.length; index < len; ++index) {
        if(contentProgress[index].value != ""){
            ...
            console.log(detailProgresId[index].value);
            detailProgres.id = detailProgresId[index].value;
    ...
    console.log("data : "+data);
    var redirectNotulensi = mappingEditNotulensi+idNotulensi;
    $.ajax({
        type : "POST",
        contentType : 'application/json; charset=utf-8',
